...
p_array = get_p_array(reward_list,delay_list,delay_datatable);
...
p_array_order = [7 4 1 8 5 2 9 6 3]; % for 3 (delay) x 3 (rwd) design
...
plot(1:length(p_array),p_array(p_array_order),'*-','LineWidth',2)
xlim([0 length(p_array)+1]); ylim([0 1])
xticks([1:1:length(p_array)]); xticklabels(label(p_array_order))
