function sva = aatradeoff_subjective_value_analysis_v04(choice_and_neural,varargin)
% appetitive-aversive tradeoff subjective value analysis
% original version by ESBM 2021-10-04
% 2021-10-15 v2 to include values derived from fit to full big behavioral
% dataset from each animal
%
% Required input: 
...
%
%  (the following columns are optional; if not included, does not do this
%   analysis)
%   Col 11 - firing rate response to chosen offer (window 1)
%   Col 12 - firing rate response to chosen offer (window 2)
%
...
do_plot = 0;
use_values_from_existing_sva = 0;
existing_sva = [];
neural_resp_param = struct('type',{{'offer1','offer2','chosenoffer','chosenoffer'}},'column',[6 7 11 12]);
...
while vi <= numel(varargin)
    if ischar(varargin{vi}) && strcmp(varargin{vi},'plot')
        ...
        vi = vi + 2;
    elseif ischar(varargin{vi}) && strcmp(varargin{vi},'use_values_from_existing_sva')
        assert(numel(varargin) >= vi+1,'expected argument to "use_values_from_existing_sva" giving the existing "sva" to use!');
        use_values_from_existing_sva = true;
        existing_sva = varargin{vi+1};
        assert(isstruct(existing_sva),'existing fit must be a struct!');
        vi = vi + 2;
    elseif ischar(varargin{vi}) && strcmp(varargin{vi},'neural_resp_param')
        assert(numel(varargin) >= vi+1,'expected argument to "neural_resp_param" giving the response & analysis params to use!');
        neural_resp_param = varargin{vi+1};
        assert(isstruct(neural_resp_param));
        vi = vi + 2;
    else
...
% get total number of neural responses we will be analyzing
neural_resp_param.n = numel(neural_resp_param.column);
assert(numel(neural_resp_param.type)==neural_resp_param.n);
...
neural_offer_response = choice_and_neural(:,neural_resp_param.column);
...
off_combo_index = nan*ones(size(off,1),2);
...
sva.param.resp = neural_resp_param;
...
sva.param.use_values_from_existing_sva = use_values_from_existing_sva;
if sva.param.use_values_from_existing_sva
    % if we are just using values from an existing set of fits, instead of
    % making new fits, it makes no sense to do cross-validation, so turn it off
    sva.param.n_crossvalidation_folds = 0;
else
...
for modi = 1:sva.model.n
    ...
    existing_fit_args = {};
    if sva.param.use_values_from_existing_sva
        % if specified, apply an existing fit instead of making a new fit
        % (e.g. if we want to derive values from a fit to a big behavioral
        % dataset including multiple sessions, to ensure that the
        % parameters (and hence the resulting values) are estimated more
        % accurately than we could from a single behavioral session's data)
        existing_fit_args = {'apply existing fit',existing_sva.model.analysis{modi}.behav_full_fit};
    end
    behav_full_fit = eglm_fit(xreg,yreg,'binary choices',name_args{:},xval_args{:},peroffer_args{:},existing_fit_args{:});
    ...
    if sva.param.do_neural_analysis
        ...
        cur.r = nan*ones(cur.n,sva.param.resp.n);
        cur.rp = nan*ones(cur.n,sva.param.resp.n);
        cur.rho = nan*ones(cur.n,sva.param.resp.n);
        cur.rhop = nan*ones(cur.n,sva.param.resp.n);
        for svi = 1:cur.n
            svname = cur.name{svi};
            if isfield(behav_full_fit.peroffer,svname) % skip if this type of value was not computed for this model
                for respi = 1:sva.param.resp.n
                    sv = nan*ones(size(behav_full_fit.peroffer.(svname),1),1);
                    switch sva.param.resp.type{respi}
                        case 'offer1'
                            oi = 1;
                        ...
                        case 'offer2'
                            ...
                            sv = behav_full_fit.peroffer.(svname)(:,oi);
                        case 'chosenoffer'
                            sv(cho==1) = behav_full_fit.peroffer.(svname)(cho==1,1);
                            sv(cho==2) = behav_full_fit.peroffer.(svname)(cho==2,2);
                        case 'nonchosenoffer'
                            sv(cho==1) = behav_full_fit.peroffer.(svname)(cho==1,2);
                            sv(cho==2) = behav_full_fit.peroffer.(svname)(cho==2,1);
                        otherwise
                            error('unknown resp type');
                    end
                    assert(~any(isnan(sv)));
                    [cur.r(svi,respi),cur.rp(svi,respi)] = corr(sv,neural_offer_response(:,respi),'type','Pearson');
                    [cur.rho(svi,respi),cur.rhop(svi,respi)] = corr(sv,neural_offer_response(:,respi),'type','Spearman');
                end
            end
        end
                ...
                if strcmp(nxreg{xi}.name,'const')
                    ...
                    for ti = 1:numel(nxreg{xi}.terms)
                        if ~ischar(nxreg{xi}.terms{ti})
                            newterm = nan*ones(size(nxreg{xi}.terms{ti},1),1);
                            switch sva.param.resp.type{respi}
                                case 'offer1'
                                    oi = 1;
                                    newterm = nxreg{xi}.terms{ti}(:,oi);
                                case 'offer2'
                                    oi = 2;
                                    newterm = nxreg{xi}.terms{ti}(:,oi);
                                case 'chosenoffer'
                                    newterm(cho==1) = nxreg{xi}.terms{ti}(cho==1,1);
                                    newterm(cho==2) = nxreg{xi}.terms{ti}(cho==2,2);
                                case 'nonchosenoffer'
                                    newterm(cho==1) = nxreg{xi}.terms{ti}(cho==1,2);
                                    newterm(cho==2) = nxreg{xi}.terms{ti}(cho==2,1);
                                otherwise
                                    error('unknown resp type');
                            end
                            nxreg{xi}.terms{ti} = newterm;
                            %nxreg{xi}.terms{ti} = nxreg{xi}.terms{ti}(:,oi);
                        end
                    end
                end
            end
            nyreg = struct('name',sprintf('offer response %d (%s)',respi,sva.param.resp.type{respi}),'value',neural_offer_response(:,respi));

            name_args = {'name',sprintf('neural full fit model %d (%s), offer response %d (%s)',modi,modname,respi,sva.param.resp.type{respi})};
            modan.neural_full_fit{respi} = eglm_fit(nxreg,nyreg,'normal',name_args{:},xval_args{:},peroffer_args{:});
        end
        
        % - fit neural rates with simple model as a function only of subjective values
        cur = struct();
        cur.name = {'rewandpunvalues','value','rewvalue','punvalue'}'; % separate weights for rew and pun values? Or just one weight of total value?
        cur.termnames = {{'rewvalue','punvalue'},{'value'},{'rewvalue'},{'punvalue'}};
        cur.n = numel(cur.name);
        ...
        for svi = 1:cur.n
            ...
            for respi = 1:sva.param.resp.n
                ...
                for ti = 1:numel(termnames)
                    newterm = nan*ones(size(behav_full_fit.peroffer.(termnames{ti}),1),1);
                    switch sva.param.resp.type{respi}
                        case 'offer1'
                            oi = 1;
                            newterm = behav_full_fit.peroffer.(termnames{ti})(:,oi);
                        case 'offer2'
                            oi = 2;
                            newterm = behav_full_fit.peroffer.(termnames{ti})(:,oi);
                        case 'chosenoffer'
                            newterm(cho==1) = behav_full_fit.peroffer.(termnames{ti})(cho==1,1);
                            newterm(cho==2) = behav_full_fit.peroffer.(termnames{ti})(cho==2,2);
                        case 'nonchosenoffer'
                            newterm(cho==1) = behav_full_fit.peroffer.(termnames{ti})(cho==1,2);
                            newterm(cho==2) = behav_full_fit.peroffer.(termnames{ti})(cho==2,1);
                        otherwise
                            error('unknown resp type');
                    end
                     
                    nxreg{end+1} = struct('name',termnames{ti},'terms',{{newterm}},'normalization','none');
                ...
                name_args = {'name',sprintf('neural %s fit model %d (%s), offer response %d (%s)',svname,modi,modname,respi,sva.param.resp.type{respi})};
                cur.fit{svi,respi} = eglm_fit(nxreg,nyreg,'normal',name_args{:},xval_args{:});
            end
        end
