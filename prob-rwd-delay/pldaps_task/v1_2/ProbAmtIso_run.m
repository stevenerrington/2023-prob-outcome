...
c.tick_diff = 0;
c.tick_step = 1/((max(c.rwd_delay)*1000)/8);

c.large_reward_cuesize = 9;
c.small_reward_cuesize = 1;
c.RewardRange_outcome = 0;
c.timer_start = 0;
...
while  ~any(t.state == t.endStates) && c.quit == 0
    ...
    switch t.state
        ...
        case 0.5
            ...
            % !STATE: Wait half a second with both options on screen ---------------------
        case 0.55
            if (GetSecs - delayvar) >= 0.5
                t.state =0.6;
                delayvar=GetSecs;
        ...
        case 0.6
            ...
            % If the monkey is out of either target window, then reset the timer.
            if ~checkEye(c.passEye, t.ang1-[s.EyeX s.EyeY], [t.PtpWindW, t.PtpWindH]) & ~checkEye(c.passEye, t.ang2-[s.EyeX s.EyeY], [t.PtpWindW, t.PtpWindH]);
            ...
            % Option 1 (A) selected:
            if delayvar >= (tempback + s.targFixDurReq) && checkEye(c.passEye, t.ang1-[s.EyeX s.EyeY], [t.PtpWindW, t.PtpWindH])
                choice_selected = 1;
            ...
            c.chosenwindow = choice_selected; c.chosen = choice_selected;
            
            t.state                 = 0.7;
            t.strobeOnFlip.logic = true;
            t.strobeOnFlip.value = c.codes.(['window' int2str(choice_selected) 'chosen']);
            delayvar = GetSecs;
            tstate_back = 0.00557;
            ...
            % Reward amount setup
            if c.(['ActualRewardOffer' int2str(choice_selected)]) == 1 % If prob is 100%
            ...
            % Delay setup
            if c.(['ActualDelayOffer' int2str(choice_selected)]) == 1 % If prob is 100%
        ...
        case 1.0
            c.rwd_reveal_flag = 1;
            tstate_back=0.005575; % !SE: MAKE A NEW SCREEN
            delayvar = GetSecs;
...
if  c.showfirst>-1
    ...
    if DelayRange == 0; DelayRange = 2;...
    elseif DelayRange == 5; DelayRange = 5.00;...
    elseif DelayRange == 10; DelayRange = 8;...
    end

if RewardRange == 0; RewardRange = c.small_reward_cuesize;...
elseif RewardRange == 5; RewardRange = 5.00;...
elseif RewardRange == 10; RewardRange = c.large_reward_cuesize;...
end
...
switch c.chosen
    case 1
        ...
        if c.PunishmentRange1 == 0; DelayRange = 2;
        elseif c.PunishmentRange1 == 5; DelayRange = 5;
        elseif c.PunishmentRange1 == 10; DelayRange = 8;
        end
        
        if c.RewardRange1 == 0; RewardRange = c.small_reward_cuesize;
        elseif c.RewardRange1 == 5; RewardRange = 5.00;
        elseif c.RewardRange1 == 10; RewardRange = c.large_reward_cuesize;
        end
        
    case 2
        target_angle = t.ang2;
        
        if c.PunishmentRange2 == 0; DelayRange = 2;
        elseif c.PunishmentRange2 == 5; DelayRange = 5;
        elseif c.PunishmentRange2 == 10; DelayRange = 8;
        end
        
        if c.RewardRange2 == 0; RewardRange = c.small_reward_cuesize;
        elseif c.RewardRange2 == 5; RewardRange = 5.00;
        elseif c.RewardRange2 == 10; RewardRange = c.large_reward_cuesize;
        end
end
...
if  c.punishdel==0
    ...
    Screen('FillRect',c.window, convertColorToL48D(colors),repmat(target_angle  + c.middleXYtt,1,2) + [-PtpWindW -PtpWindH PtpWindW PtpWindH*0],1)
    n=c.maxValrangeOf1P;
    PtpWindW    =  deg2pix(1,c); % target point window width
    PtpWindH    =  deg2pix(n,c); % target point window height
    Screen('FrameRect',c.window, convertColorToL48D(colors),repmat(target_angle  + c.middleXYtt,1,2) + [-PtpWindW -PtpWindH PtpWindW PtpWindH*0],5)
end

colors=10;
PtpWindW    =  deg2pix(1,c); % target point window width
PtpWindH    =  deg2pix(RewardRange,c); % target point window height
Screen('FillRect',c.window, convertColorToL48D(colors),repmat(target_angle  + c.middleXYt,1,2) + [-PtpWindW -PtpWindH PtpWindW PtpWindH*0],1)
n=c.maxValrangeOf1R;
PtpWindW    =  deg2pix(1,c); % target point window width
PtpWindH    =  deg2pix(n,c); % target point window height
Screen('FrameRect',c.window, convertColorToL48D(colors),repmat(target_angle  + c.middleXYt,1,2) + [-PtpWindW -PtpWindH PtpWindW PtpWindH*0],5)
...
if (max_val_range_pun*offer_pun) == 0
    c.timer_start = 2; c.short_delay_flag = 1; c.long_delay_flag = 0;
else
    c.timer_start = 8; c.short_delay_flag = 0; c.long_delay_flag = 1;
end

if reward_range == 0; RewardRange = c.small_reward_cuesize;
elseif reward_range == 5; RewardRange = 5.00;
elseif reward_range == 10; RewardRange = c.large_reward_cuesize;
end

if offer_rwd == 0; c.RewardRange_outcome = c.small_reward_cuesize;
else c.RewardRange_outcome = c.large_reward_cuesize;
end
...
elseif  c.punishdel==1
    ...
    elseif c.rwd_reveal_flag == 1
        ...
        colors=10;
        PtpWindW    =  deg2pix(1,c); % target point window width
        PtpWindH    =  deg2pix(c.RewardRange_outcome,c); % target point window height
        Screen('FillRect',c.window, rwd_reveal_color,repmat(targ_angle_in  + c.middleXYt,1,2) + [-PtpWindW -PtpWindH PtpWindW PtpWindH*0],1)
...
if c.j~=0
    ...
    %   Option 1:
    PDS.offerInfo{1}.choice_rwd(c.j) = c.RewardRange1;
    PDS.offerInfo{1}.choice_delay(c.j) = c.PunishmentRange1;
    %   Option 2:
    PDS.offerInfo{2}.choice_rwd(c.j) = c.RewardRange2;
    PDS.offerInfo{2}.choice_delay(c.j) = c.PunishmentRange2;
    
    % Outcome
    PDS.rwdOutcomeInfo(c.j) = c.RewardRange_outcome;
    PDS.delayOutcomeInfo(c.j) = c.timer_start;
    
    PDS.offer_settings.maxValrangeOf1P(c.j) = c.maxValrangeOf1P;
    PDS.offer_settings.maxValrangeOf1R(c.j) = c.maxValrangeOf1R;
    PDS.offer_settings.maxValrangeOf2P(c.j) = c.maxValrangeOf2P;
    PDS.offer_settings.maxValrangeOf2R(c.j) = c.maxValrangeOf2R;
    PDS.offer_settings.offer1_pun(c.j) = c.Offer1Pun;
    PDS.offer_settings.offer1_rwd(c.j) = c.Offer1Rew;
    PDS.offer_settings.offer2_pun(c.j) = c.Offer2Pun;
    PDS.offer_settings.offer2_rwd(c.j) = c.Offer2Rew;
    
    %   Selection:
    PDS.whichtoshowfirst(c.j) = c.whichtoshowfirst;     % First shown option (1 or 2)
    PDS.chosenwindow(c.j)     = c.chosenwindow;         % Selected option
    PDS.iti_dur(c.j)     = c.chosenwindow;         % Selected option
    ...
    PDS.intrareveal_interval(c.j)      = c.intrareveal_interval;      % Laser duration
    PDS.reveal_outcome_interval(c.j)            = c.reveal_outcome_interval;            % ITI duration
    %
    ...
    %   Delay time
    try PDS.timedelay(c.j)= s.TimeofPunish;
    catch PDS.timedelay(c.j)= NaN;
    end
    ...
    %   Delay magnitude
    try PDS.magnitude_delay(c.j)= s.rewardDelay_;
    catch PDS.magnitude_delay(c.j)= NaN;
    end
    
    %   Delay magnitude
    try PDS.magnitude_reward(c.j)= c.rewardDur;
