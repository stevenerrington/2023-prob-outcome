...
oku = ~any(isnan(res.loglik),2);

% NOTE TO ILYA:
% put a line here further restricting 'oku' to select the subset of
% units you want to analyze. E.g.:
%
% oku = oku & ... ;
...
for vari = 1:numel(varinfo)
    ...
    for rowi = 1 %:2
        ...
        h(rowi,vari)=nsubplot(1,numel(varinfo),rowi,vari);
       ...
       histogram(x);
        ...
        etextn('rt',sprintf('mean %f\nsignrank p = %f\nn=%d',mean(x),signrank(x),numel(x)));
    end
end
