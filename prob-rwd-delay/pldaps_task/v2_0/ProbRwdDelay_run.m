...
;
offer_delay = c.(['Offer' int2str(c.chosen) 'delay']);
reward_range = c.(['RwdMagnitude' int2str(c.chosen)]);
delay_range = c.(['DelayMagnitude' int2str(c.chosen)]);
max_val_range_rwd = c.maxValrange;
offer_rwd = c.(['Offer' int2str(c.chosen) 'Rew']);
...
if (c.maxValrange*offer_delay) == 0
...
% Delay ----------------------
PtpWindW    =  deg2pix(1,c); % target point window width
PtpWindH    =  deg2pix(c.maxValrange*offer_delay,c); % target point window height
c.middleXYtt=c.middleXYt; c.middleXYtt(1)=c.middleXYtt(1)+70;
